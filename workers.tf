resource "azurerm_network_interface" "kthw-worker-nics" {
  name                = "kthw-worker-${count.index}-nic"
  location            = "${azurerm_resource_group.kthw-rg.location}"
  resource_group_name = "${azurerm_resource_group.kthw-rg.name}"

  count = 3

  enable_ip_forwarding = true

  internal_dns_name_label = "kthw-worker-${count.index}"

  ip_configuration {
    name                          = "kthw-worker-${count.index}-ip-config"
    subnet_id                     = "${azurerm_subnet.kthw-subnet1.id}"
    private_ip_address_allocation = "static"
    private_ip_address            = "10.240.0.1${count.index}"
  }
}

resource "azurerm_virtual_machine" "kthw-workers" {
  name                  = "kthw-worker-${count.index}"
  location              = "${azurerm_resource_group.kthw-rg.location}"
  resource_group_name   = "${azurerm_resource_group.kthw-rg.name}"
  network_interface_ids = ["${azurerm_network_interface.kthw-worker-nics.*.id[count.index]}"]
  vm_size               = "Standard_B2ms"

  count = 3

  availability_set_id = "${azurerm_availability_set.kthw-worker-as.id}"

  # Uncomment this line to delete the OS disk automatically when deleting the VM
  delete_os_disk_on_termination = true

  # Uncomment this line to delete the data disks automatically when deleting the VM
  delete_data_disks_on_termination = true

  storage_image_reference {
    publisher = "Canonical"
    offer     = "UbuntuServer"
    sku       = "16.04-LTS"
    version   = "latest"
  }

  storage_os_disk {
    name              = "kthw-worker-${count.index}-osdisk"
    caching           = "ReadWrite"
    create_option     = "FromImage"
    managed_disk_type = "Standard_LRS"
    disk_size_gb      = 200
  }

  # Optional data disks

  os_profile {
    computer_name  = "kthw-worker-${count.index}"
    admin_username = "kubeheinz"
    admin_password = "Password1234!"
  }
  os_profile_linux_config {
    disable_password_authentication = true

    ssh_keys {
      path     = "/home/kubeheinz/.ssh/authorized_keys"
      key_data = "ssh-rsa  marrat@MARRAT-PC"
    }
  }
  tags {
    environment = "KTHW"
  }
}
